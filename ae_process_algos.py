...
def singal_to_Spectrum(
    ...
    return np.fft.rfft(signal - np.mean(signal))
...
def compute_all_ae_processing_algos(signal,sample_rate,lower_frequency,higher_frequency,threshold,roll_off):
        ...
        spectrum = singal_to_Spectrum(signal)

        return pd.Series({
                "band_energy": band_energy(spectrum,sample_rate,lower_frequency,higher_frequency),
                "band_energy_ratio": band_energy_ratio(spectrum,sample_rate,lower_frequency,higher_frequency),
                "clearance_factor": clearance_factor(signal),
                "counts": counts(signal,threshold),
                "crest_factor": crest_factor(signal),
                "energy": energy(signal),
                "impulse_factor": impulse_factor(signal),
                "k_factor": k_factor(signal),
                "kurtosis": kurtosis(signal),
                "margin_factor": margin_factor(signal),
                "peak_amplitude": peak_amplitude(signal),
                "rms": rms(signal),
                "shape_factor": shape_factor(signal),
                "skewness": skewness(signal),
                "spectral_centroid": spectral_centroid(spectrum,sample_rate),
                "spectral_kurtosis": spectral_kurtosis(spectrum,sample_rate),
                "spectral_peak_frequency": spectral_peak_frequency(spectrum,sample_rate),
                "spectral_rolloff": spectral_rolloff(spectrum,sample_rate,roll_off),
                "spectral_skewness": spectral_skewness(spectrum,sample_rate),
                "spectral_variance": spectral_variance(spectrum,sample_rate),
                "zero_crossing_rate": zero_crossing_rate(signal,sample_rate)
                })
