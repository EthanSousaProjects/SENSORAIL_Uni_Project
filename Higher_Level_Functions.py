# Will now serve as a place for misc functions
...
# Ultimately needs to be included in the dataset class and needs to retrieve both the data and parameters from the disk
...
def get_most_recent_csv(directory: str):
    ...
    # Filtering out files with "Parameters" in the file name
    filtered_files = [f for f in csv_files if "Parameters" not in f]
...
# ------------------------------
# Old functions commented out for now.
# ------------------------------
...
# def Measure_Mod(redp, eFreq: list[int],
#             amp: list[float], 
#             inChannel: int, 
#             outChannel: int, 
#             writeToDisk: bool = False,
...
#             # Starting the signal, collecting the data, and stopping the signal
#             [sig1, sig2] = redp.pulseAndRecieve(outChannel, freq/1000, current_amp)

#             # Appending the data to the output list 
#             now = datetime.datetime.now()
#             outList.append([freq, iter, current_amp, now.time(), sig1])
